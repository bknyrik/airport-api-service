...
class AuthenticatedAirplaneApiTests(APITestCase):
    ...
    def test_airplane_upload_image_is_forbidden(self) -> None:
        with tempfile.NamedTemporaryFile(suffix=".jpg") as ntf:
            URL = get_airplane_upload_image_url(pk=1)
...
class AdminAirplaneApiTests(APITestCase):
    ...
    def test_airplane_create_has_bad_request(self) -> None:
        data = {
            "name": "Test airplane",
            "rows": -1,
            "seats_in_row": 20,
            "airplane_type": self.airplane_type.id,
            "facilities": (self.facility_1.id,)
        }
        response = self.client.post(AIRPLANE_LIST_URL, data=data)
    ...
    def test_airplane_update_has_bad_request(self) -> None:
        data = {
            "name": "Another test airplane",
            "rows": 2,
            "airplane_type": 999,
    ...
    def test_airplane_partial_update(self) -> None:
        data = {"rows": 6, "seats_in_row": 45}
        URL = get_airplane_detail_url(pk=self.airplane.id)
        response = self.client.patch(URL, data=data)
        serializer = AirplaneSerializer(self.airplane)
