...
class AuthenticatedAirportApiTests(APITestCase):
    ...
    def test_airport_partial_update_is_forbidden(self) -> None:
        data = {"description": "Another test description"}
        response = self.client.patch(get_airport_detail_url(pk=1), data=data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class AuthenticatedAdminAirportApiTests(APITestCase):
    ...
    def test_airport_create_has_bad_request(self) -> None:
        data = {
            "name": "Test airport",
            "country": "Test country",
            "city": "Test city",
    ...
    def test_airport_update_has_bad_request(self) -> None:
        airport = airport_sample()
        data = {
            "name": "Updated airport",
            "country": "Updated country",
    ...
    def test_airport_partial_update_has_bad_request(self) -> None:
        airport = airport_sample()
        data = {"iata_code": "XRT C"}
        response = self.client.patch(
            get_airport_detail_url(pk=airport.id),
            data=data
        )
